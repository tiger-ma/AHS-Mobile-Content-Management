...
<sql:query var="empList" dataSource="jdbc/TestDB" scope="request">
  SELECT * FROM feed WHERE status = '1'
...
<form action = "feed-add-new.jsp">
	<h1>Add a New Article/Post to Feed</h1>
...
<table border="1">
	...
	<tr>
		...
		<td>
		<select  style="color: red" name="type">
		    <option value="0" disabled="disabled" selected="selected">Please select a type.</option>
		...
		<td>
			<input type="submit" style="color: red" value="Add to Feed" />
		</td>	
	</tr>
</table>
</br>
	...
	<table border="1">
		...
		<sql:query var="imageList" dataSource="jdbc/TestDB" scope="request">
  		SELECT * FROM image WHERE uniqueid = ? order by create_time asc
			<sql:param value="feed ${fn:escapeXml(row.id)}" /> 
		</sql:query>
  		...
  		SELECT * FROM notification2 WHERE uniqueid = ? ORDER BY create_time desc
			...
			<tr>
					...
					<td>
						...
						<form action="feed-image-add.jsp" method="post">
					...
					<td>
						<form action="feed-image-delete.jsp" method="post">
							<input type="hidden" name="feedid"
						    value="${fn:escapeXml(row.id)}">
					...
					<td><textarea readonly cols="10" rows="5">${fn:escapeXml(row.type)}</textarea></td>
					...
					<td>
							...
							<form action="feed-add-notification.jsp" method="post">
					...
					<td>
							<form action="feed-notification-delete.jsp" method="post">
							<input type="hidden" name="feedid"
						    value="${fn:escapeXml(row.id)}">
							<input type="submit" style="color: red" value="Clear List">
						</form>
					...
					<td>
						<form action="feed-delete.jsp" method="post">
						  <input type="hidden" name="feedid"
						    value="${fn:escapeXml(row.id)}">
