...
<c:if test="${ empty param.type || empty param.org || empty param.title || empty param.author || empty param.date || empty param.article}">
  <c:redirect url="feed.jsp" >
	...
	<sql:transaction dataSource="jdbc/TestDB">
		<sql:update >
		  insert into feed (type, org, title, author, date, caption, article, create_time, status) values (?, ?, ?, ?, ?, ?, ?, sysdate(), '1')
