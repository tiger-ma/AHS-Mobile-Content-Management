...
<c:if test="${ empty param.link || empty param.title || empty param.author || empty param.date || empty param.article}">
  <c:redirect url="home.jsp" >
	...
	<sql:transaction dataSource="jdbc/TestDB">
		<sql:update >
		  insert into home (link, title, author, date, caption, article, create_time, status) values (?, ?, ?, ?, ?, ?, sysdate(), '0')
	...
	<c:redirect url="home.jsp" >
			<c:param name="reminderMsg" 
		      value="You have successfully submitted a request." />
	            </c:redirect>
